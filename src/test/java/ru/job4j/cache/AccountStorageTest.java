...
class AccountStorageTest {
    ...
    @Test
    void whenTransferAndNotEnoughAmountThenFalse() {
        var storage = new AccountStorage();
        storage.add(new Account(1, 100));
        storage.add(new Account(2, 100));
    ...
    @Test
    void whenTransferAndToAccountNotFoundThenFalse() {
        var storage = new AccountStorage();
    ...
    @Test
    void whenTransferAndFromAccountNotFoundThenFalse() {
        var storage = new AccountStorage();
        storage.add(new Account(2, 200));
        boolean result = storage.transfer(1, 2, 100);
        var account = storage.getById(2)
                .orElseThrow(() -> new IllegalStateException("Not found account by id = 2"));
        assertThat(storage.getById(1)).isEmpty();
        assertThat(account.amount()).isEqualTo(200);
        assertThat(result).isFalse();
